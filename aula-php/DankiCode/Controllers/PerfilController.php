      ...
      if(isset($_SESSION['login'])){
          if(isset($_POST['atualizar'])){
            ...
            if($nome == '' || strlen($nome) < 3){
              // \DankiCode\Utilidades::alerta('Precisa inserir um nome');
            ...
            if($senha != ''){
              ...
              $_SESSION['nome'] = $nome;
              // \DankiCode\Utilidades::alerta('Senha alterada com sucesso!');
							// \DankiCode\Utilidades::redirect(INCLUDE_PATH.'perfil');
            }else{
              $atualizar = $pdo->prepare("UPDATE usuarios SET nome = ? WHERE id = ?");
              $atualizar->execute(array($nome,$_SESSION['id']));
              $_SESSION['nome'] = $nome;
              // \DankiCode\Utilidades::alerta('Nome alterado com sucesso!');
							// \DankiCode\Utilidades::redirect(INCLUDE_PATH.'perfil');
            ...
            if($_FILES['file']['tmp_name'] != ''){
              ...
              if($fileExt == 'png' || $fileExt == 'jpg' ||$fileExt == 'jpeg'){
                ...
                if($size <= 300){
                  ...
                  $_SESSION['img'] = $uniqid;
                  move_uploaded_file($file['tmp_name'],'C:\xampp\htdocs\aula-php/uploads/'.$uniqid);
